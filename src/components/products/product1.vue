  ...
  export default{
    ...
    watch: {
      ...
      beforeCombination(value){
        ...
        const result = combinaciones.filter((combinacion, index) => JSON.stringify(combinacion.combinacion) == combinationSelected)[0];
        this.productCart = [];
    ...
    methods: {
      searchIdForSlug() {
        const product = this.productsData.filter(product => product.slug === this.$route.params.slug);
        if(product.length) {
          return product[0].id
        }
        return 3549
